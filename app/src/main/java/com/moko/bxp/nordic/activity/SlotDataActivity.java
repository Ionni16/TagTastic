...
public class SlotDataActivity extends BaseActivity implements NumberPickerView.OnValueChangeListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //ToastUtils.showToast(SlotDataActivity.this, "mio slot interno data !");
        // AP 2025-04-16
        BaseApplication myApp = (BaseApplication) getApplication();
        if (myApp.GetMTAutoWriteStatus() == 1) {
            // Chiama la funzione per selezionare automaticamente URL sul type
            //this.post rlSlot2.postDelayed(new Runnable() { rlSlot2.performClick(), 3000}); // era 1000
            //this.onPostCreate();
            // onValueChange(npvSlotType, 0, 1);
            new Handler().postDelayed(new Runnable() {
                @Override
                public void run() { ImpostazioneTipoURL_1(); }
            }, 1000); // in millisecondi (n secondi) di ritardo
        }

        // === Mitech AutoWrite: avvio automatico configurazione URL ===
        BaseApplication app = (BaseApplication) getApplication();
        int st = app.GetMTAutoWriteStatus();
        if (st == 11 || st == 1) {
            // Aspetta davvero che i fragment abbiano la view pronta
            scheduleAutoConfigureUrl();
        }


    }

    private void autoConfigureUrl() {
        // Se la activity è già in chiusura evita crash
        if (isFinishing()) return;

        // 2 = URL (nel tuo array è URL)
        npvSlotType.setValue(2);
        onValueChange(npvSlotType, 0, 2);

        if (urlFragment != null) {
            urlFragment.SetMitechValoriTag();
        }

        // prossimo step: password
        BaseApplication app = (BaseApplication) getApplication();
        app.SetMTAutoWriteStatus(2);

        new Handler().postDelayed(() -> {
            if (!isFinishing()) ivSave.performClick();
        }, 500);
    }

    private int mAutoTry = 0;

    private void scheduleAutoConfigureUrl() {
        mAutoTry = 0;
        frameSlotContainer.postDelayed(this::autoConfigureUrlSafe, 200);
    }

    private void autoConfigureUrlSafe() {
        if (isFinishing()) return;

        // Assicura che le transazioni dei fragment siano applicate
        try {
            getFragmentManager().executePendingTransactions();
        } catch (Exception ignored) {}

        // URL index = 2 nel tuo array (come mi hai scritto)
        npvSlotType.setValue(2);
        onValueChange(npvSlotType, 0, 2);

        // Ora urlFragment DEVE essere pronto (view != null)
        boolean ready = (urlFragment != null && urlFragment.isAdded() && urlFragment.getView() != null);

        if (!ready) {
            mAutoTry++;
            if (mAutoTry < 15) {
                // retry fino a ~3 secondi totali
                frameSlotContainer.postDelayed(this::autoConfigureUrlSafe, 200);
            } else {
                ToastUtils.showToast(this, "Autowrite: URL fragment non pronto (timeout).");
            }
            return;
        }

        // Imposta valori e salva
        urlFragment.SetMitechValoriTag();

        BaseApplication app = (BaseApplication) getApplication();
        app.SetMTAutoWriteStatus(2); // step successivo

        frameSlotContainer.postDelayed(() -> {
            if (!isFinishing()) ivSave.performClick();
        }, 400);
    }
    ...
    private void ImpostazioneTipoURL_1() {
        // Il codice che vuoi eseguire dopo che la UI è stata disegnata
        ToastUtils.showToast(SlotDataActivity.this, "esecuzione Post.");
        npvSlotType.setValue(2); // 2 -> URL
        onValueChange(npvSlotType, 0, 2);
        if (urlFragment != null)
            urlFragment.SetMitechValoriTag();
        BaseApplication myApp = (BaseApplication) getApplication();
        myApp.SetMTAutoWriteStatus(2); // al ritorno devo uscire, altrimenti va in loop
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() { ivSave.performClick(); }
        }, 500); // Ritardo di 1 secondo (1000 millisecondi)
    }
    private void ImpostazioneTipoURL_2() {
        // Il codice che vuoi eseguire dopo che la UI è stata aggiornata
        if (urlFragment != null)
            urlFragment.SetMitechValoriTag();
    }
